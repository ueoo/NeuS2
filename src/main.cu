...
int main(int argc, char **argv) {
    ...
    try {
        ...
        // Track total planned training iterations for logging purposes (best-effort estimate)
        ...
        if (snapshot_flag) {
            ...
            // Estimate total training iterations from config/hyperparams if available
            if (testbed.first_frame_max_training_step > 0) {
                if (testbed.all_training_time_frame > 1 && testbed.next_frame_max_training_step > 0) {
                    total_training_steps = testbed.first_frame_max_training_step + (testbed.all_training_time_frame - 1) * testbed.next_frame_max_training_step;
                } else {
