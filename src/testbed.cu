    ...
    if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::TreeNode("Advanced camera settings")) {
            ...
            snprintf(buf, sizeof(buf),
                     ...
                     "testbed.autofocus = %s\n\n",
                     b.x(), b.y(), b.z(), b.w(), m_exposure, s.x(), s.y(), s.z(), u.x(), u.y(), u.z(), v.x(), v.y(), v.z(), p.x(), p.y(), p.z(), scale(), fov(), m_dof, m_slice_plane_z, m_autofocus_target.x(), m_autofocus_target.y(), m_autofocus_target.z(), m_autofocus ? "True" : "False");
            ...
            if (m_testbed_mode == ETestbedMode::Sdf) {
                ...
                snprintf(buf + n, sizeof(buf) - n,
                         ...
                         m_sdf.shadow_sharpness, m_sdf.analytic_normals ? "True" : "False", m_sdf.use_triangle_octree ? "True" : "False", m_sdf.brdf.metallic, m_sdf.brdf.subsurface, m_sdf.brdf.specular, m_sdf.brdf.roughness, m_sdf.brdf.sheen, m_sdf.brdf.clearcoat, m_sdf.brdf.clearcoat_gloss, m_sdf.brdf.basecolor.x(), m_sdf.brdf.basecolor.y(), m_sdf.brdf.basecolor.z());
            }
...
void Testbed::draw_visualizations(ImDrawList *list, const Matrix<float, 3, 4> &camera_matrix) {
    ...
    if (m_testbed_mode != ETestbedMode::Image) {
        ...
        view2proj << xyscale, 0, (float)m_window_res.x() * screen_center.x() * zscale, 0,
        ...
        if (shift) {
            ...
            Matrix3f rot =
                (AngleAxisf(static_cast<float>(-rel.x() * 2 * PI() * rot_sensitivity), up) * // Scroll sideways around up vector
                 AngleAxisf(static_cast<float>(-rel.y() * 2 * PI() * rot_sensitivity), side))
