...
NerfDataset load_nerf(const std::vector<filesystem::path> &jsonpaths, float sharpen_amount) {
    ...
    for (size_t i = 0; i < jsons.size(); ++i) {
        ...
        if (json.contains("frames") && json["frames"].is_array())
            pool.parallelForAsync<size_t>(0, json["frames"].size(), [&progress, &n_loaded, &result, &images, &json, basepath, image_idx, info, rolling_shutter, principal_point, camera_distortion, part_after_underscore, fix_premult, enable_depth_loading, enable_ray_loading](size_t i) {
			...
			progress.update(++n_loaded); }, futures);
