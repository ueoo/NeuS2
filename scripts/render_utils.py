...
def render_img_training_view(args, testbed, log_ptr, image_dir, frame_time_id=0, training_step=-1):
    ...
    if training_step < 0:
        ...
        write_image(
            join(img_path, f"frame_{frame_time_id:06}", f"frame_{frame_time_id:06}_{training_step}_gt.png"), ref_image
        )
    ...
    if training_step < 0:
        ...
        write_image(
            join(img_path, f"frame_{frame_time_id:06}", f"frame_{frame_time_id:06}_{training_step}_pred.png"), image
        )
    ...
    if training_step < 0:
        ...
        write_image(
            join(img_path, f"frame_{frame_time_id:06}", f"frame_{frame_time_id:06}_{training_step}_diff.png"), diffimg
        )
        return
    ...
    if args.save_mesh:
        ...
        if "intrinsic_matrix" in frame:
            ...
            if "fl_x" in test_transforms or "fl_y" in test_transforms:
                ...
                fl_x = (
                    W / (2 * np.tan(test_transforms["camera_angle_x"] / 2))
                    if "camera_angle_x" in test_transforms
                    else None
                )
                fl_y = (
                    H / (2 * np.tan(test_transforms["camera_angle_y"] / 2))
                    if "camera_angle_y" in test_transforms
                    else None
                )
                if fl_x is None:
                    fl_x = fl_y
            ...
            ixt = torch.tensor([[fl_x, 0, cx, 0], [0, fl_y, cy, 0], [0, 0, 1, 0], [0, 0, 0, 1]])
