...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Save this snapshot after training every frame. recommended extension: .msgpack",
    )
...
if __name__ == "__main__":
    ...
    if args.dynamic_test:
        ...
        while True:
            if os.path.isdir(args.scene):
                all_transform_path = sorted(
                    list(
                        filter(lambda path: "downsample" not in path.split("/")[-1], glob.glob(args.scene + "/*.json"))
                    )
                )
            else:
            ...
            if args.load_snapshot != "":
                load_snapshot_path = (
                    os.path.join(args.load_snapshot, "checkpoints")
                    + f"/frame_{testbed.current_training_time_frame}.msgpack"
                )
            else:
                load_snapshot_path = (
                    os.path.join(args.output_path, "checkpoints")
                    + f"/frame_{testbed.current_training_time_frame}.msgpack"
                )
            ...
            if args.dynamic_save_mesh:
                ...
                save_mesh_path = os.path.join(
                    args.output_path, "evaluation", "mesh", f"frame_{testbed.current_training_time_frame:04}.obj"
                )
                ...
                if args.dynamic_save_mesh_only:
                    if (
                        not testbed.training_network_next_frame()
            ...
            log_ptr, avg_psnr_this_frame = cal_psnr(
                ...
                save_dir=os.path.join(
                    args.output_path, "evaluation", "psnr", f"frame_{testbed.current_training_time_frame}"
                ),
                spp=8,
            ...
            with open(
                os.path.join(
                    args.output_path, "evaluation", f"dynamic_{testbed.current_training_time_frame}_eval_log.json"
                ),
                "w",
            ) as f:
        ...
        if os.path.isdir(args.scene):
            all_transform_path = sorted(
                list(filter(lambda path: "downsample" not in path.split("/")[-1], glob.glob(args.scene + "/*.json")))
            )
        else:
            all_transform_path = [args.scene]
        all_pred_img = []
        ...
        if n_steps > 0:
            ...
            while testbed.frame():
                ...
                if (
                    testbed.current_training_time_frame == 0
                    and testbed.training_step == testbed.first_frame_max_training_step
                ) or (
                    testbed.current_training_time_frame != 0
                    and testbed.training_step == testbed.next_frame_max_training_step
                ):
                    ...
                    if args.save_snapshot_per_frame:
                        save_snapshot_path = os.path.join(
                            args.output_path, "checkpoints", f"frame_{testbed.current_training_time_frame}.msgpack"
                        )
                    ...
                    save_transform_path = os.path.join(
                        args.output_path, "pred_transform", f"frame_{testbed.current_training_time_frame}.txt"
                    )
                    ...
                    if args.save_mesh:
                        res = args.marching_cubes_res
                        save_mesh_path = os.path.join(
                            args.output_path, "mesh", f"frame_{testbed.current_training_time_frame:04}.obj"
                        )
                        print(
                            f"Generating mesh via marching cubes and saving to {save_mesh_path}. Resolution=[{res},{res},{res}]"
                        )
                    ...
                    if testbed.current_training_time_frame != testbed.all_training_time_frame - 1:
                        ...
                        if args.test_psnr:
                            ref_images = load_ref_images(
                                args, all_transform_path[testbed.current_training_time_frame + 1]
                            )
                    now = time.monotonic()
                    print_now = time.monotonic()
